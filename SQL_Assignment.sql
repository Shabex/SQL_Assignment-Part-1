...
select sum(total_amount) sum_amount
...
select first_name,last_name,sum(s.total_amount) cust_spent
	from assignment.customers c 
		join assignment.sales s 
		using(customer_id)
	group by first_name,last_name
	order by cust_spent desc;
...
select 
		c1.first_name  AS first1_name,
	    c1.last_name AS last1_name,
	    c1.membership_status,
	    c2.first_name AS first2_name,
	    c2.last_name  AS last2_name
...
select product_name, sum(s.total_amount ) sum_of_sales
	from assignment.sales s 
		join assignment.products p 
		using(product_id)
...
select first_name, last_name,p.category 
	from assignment.customers c
		join assignment.sales s
		using(customer_id)
		join assignment.products p 
...
select first_name,last_name, s.total_amount
	from assignment.customers c
	join assignment.sales s 
		using(customer_id)
	where extract(year from s.sale_date) = 2023
...
select p.product_name,category,price
	from assignment.products p
	join assignment.sales s 
		using(product_id )
	order by price desc
...
select membership_status, sum(total_amount) as total_sales
	from assignment.customers 
	 join assignment.sales 
	 	using(customer_id)
	 join assignment.products 
	 	using(product_id)
...
select c.first_name,sum(s.quantity_sold ) pdct_bought
	from assignment.customers c
		join assignment.sales s 
		using(customer_id)
...
select product_id,avg(quantity_sold) as average
	from assignment.sales s 
	group by product_id
...
select count(*) as no_of_sales
	from assignment.sales s
	where extract(year from s.sale_date)=2023 and extract(month from s.sale_date)=12;
...
select distinct(product_id),i.stock_quantity  
...
select product_id, s.total_amount 
	from assignment.sales s
	order by s.total_amount desc;

-- 35. Write a query to find all customers who bought products within 7 days of their registration date.

select c.first_name,c.last_name, (s.sale_date-c.registration_date) days_post_reg
	from assignment.sales s 
	join assignment.customers c 
		using(customer_id)
	where (s.sale_date-c.registration_date) <=7 and (s.sale_date-c.registration_date)>0 ;
...
select product_name,price
	from assignment.sales s
	left join assignment.products p 
		using(product_id)
...
--				c.last_name last_name, 
--				s.quantity_sold qty, 
--				dense_rank()over(order by s.quantity_sold desc) as sale_rank
--			from assignment.sales s
--			join assignment.customers c 
--				using(customer_id)
--		)
--	select concat(first_name,' ',last_name) full_name, qty
--		from cte
--		where sale_rank = 1;
...
(select product_id, max(i.stock_quantity ) stock, 'Highest Stock' stock_type
	from assignment.inventory i
	group by product_id
	order by stock desc
	limit 1)
union all
(select product_id, max(i.stock_quantity )stock, 'Lowest Stock' stock_type
	from assignment.inventory i
	group by product_id
	order by stock
	limit 1);
...
select concat(first_name,' ',last_name),sum(s.total_amount ) as total_sales
	from assignment.sales s 
	left join assignment.customers c 
		using(customer_id)
	group by first_name, last_name,s.total_amount 
	order by s.total_amount  desc
	limit 5;
